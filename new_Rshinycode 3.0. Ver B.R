    ...
    tabPanel("Cleaning & Preprocessing", icon = icon("broom"),
             ...
             tabsetPanel(id = "cleaning_tabs",
                         ...
                         tabPanel("Check Duplicates",
                                  titlePanel("Identify and Handle Duplicate Rows"),
                                  sidebarLayout(
                                    sidebarPanel(
                         ...
                         tabPanel("Drop Columns",
                                  ...
                                  actionButton("drop_vars", "Drop Selected Columns", class = "btn-primary"),  # Button to trigger the drop
                         ...
                         tabPanel("Data Preprocessing",
                                  ...
                                  sidebarLayout(
                                    sidebarPanel(
                                      ...
                                      actionButton("finish_preprocessing", "Finish Preprocessing", class = "btn-success")
                                    ),
                                    mainPanel(
...
server <- function(input, output, session) {
  ...
  observe({
    if (!is.null(data())) {
      vars_with_types <- setNames(names(data()), paste0(names(data()), " (", sapply(data(), function(x) class(x)[1]), ")")) # Version B
      updateSelectInput(session, "selectedVars", choices = vars_with_types)
  ...
  output$var_convert <- renderUI({
    vars_with_types <- setNames(names(data()), paste0(names(data()), " (", sapply(data(), function(x) class(x)[1]), ")")) # Version B
    selectInput("variable_to_convert", "Select Your Taget Variable", choices = vars_with_types)
  })
  ...
  observe({
    vars_with_types <- setNames(names(data()), paste0(names(data()), " (", sapply(data(), function(x) class(x)[1]), ")")) #Version B
    updateSelectInput(session, "target_feature", choices = vars_with_types)
  })
  ...
  observe({
    ...
    vars_with_types <- setNames(names(data()), paste0(names(data()), " (", sapply(data(), function(x) class(x)[1]), ")")) #Version B
    updateSelectInput(session, "interaction_var1", choices = vars_with_types)
    updateSelectInput(session, "interaction_var2", choices = vars_with_types)
    updateSelectInput(session, "poly_var", choices = vars_with_types)
    updateSelectInput(session, "log_var", choices = vars_with_types)
    updateSelectInput(session, "std_vars", choices = vars_with_types)
  })
